...
```sh
DEVELOPER_ID="Developer ID Application: Bradley Berkman (L65VUZN7VJ)" ./build.sh --release
ditto -c -k --keepParent dist/Writ.app dist/Writ.zip
xcrun notarytool submit dist/Writ.zip --keychain-profile AC_PASSWORD --wait
xcrun stapler staple dist/Writ.app
spctl -a -vvv -t exec dist/Writ.app      # expect: accepted
```
...
- **Menu bar managers hide new items.** Bartender parks unknown status items
  ...
  with the System Events AX query before debugging anything.
