...
if [ "${1:-}" != "--package-only" ]; then
    ...
    # site/public is generated BY this script, so a previous run leaves it
    # modified — checking it here meant every second release refused to start.
    # Source must still be clean, because the build number is the commit count.
